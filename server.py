...
async def stream(search_uuid: str, request: QueryRequest):
    ...
    if request.ask_type == "llm":
        ...
        prompt = (
            " You are a large language AI assistant develop by nash_su. Answer user question in "
            + answer_language
        ...
        generate = generator(
            prompt,
            model=request.model,
            llm_auth_token=request.llm_auth_token,
            llm_base_url=request.llm_base_url,
            using_custom_llm=request.using_custom_llm,
        )
    else:
        prompt = None
        limit_count = 10
